...
#+begin_quote warning
Read this documentation within Doom Emacs ([[kbd:][M-x doom/help]]) or online at
https://docs.doomemacs.org. It will be rendered incorrectly anywhere else (e.g.
Github).
#+end_quote
...
1. Some language modules lack LSP support (either because it doesn't exist or
   ...
   supported by [[package:lsp-mode]], simply adding [[func:lsp!]] to that major mode's hook will
   be enough to enable it:
...
- [[var:doom-big-font]]: the large font to use when [[mode:doom-big-font-mode]] is
  active.
...
Doom uses the [[package:display-line-numbers]] package, which is included with Emacs 26+.
...
Look up more precise documentation for this variable in Emacs by typing
[[kbd:<help> v display-line-numbers-type]].
...
#+begin_quote wip
*This post is a work in progress!* However, there's a post on our Discourse that
outlines [[https://discourse.doomemacs.org/t/how-does-doom-start-up-so-quickly/163/1][some of our older techniques]].
#+end_quote
